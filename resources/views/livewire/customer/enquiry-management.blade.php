                        ...
                        <td class="py-3 px-4 flex gap-2">
                           <button aria-label="View enquiry {{ $enquiry->enquiry_number }}"
                            ...
                            @if($activeTab !== 'deleted')
                                ...
                                <button wire:click="confirmDelete({{ $enquiry->id }}, '{{ $enquiry->enquiry_number }}')" 
    ...
    @if ($showDeleteModal)
        <div class="fixed inset-0 z-50 flex items-center justify-center bg-opacity-25">
            <div class="bg-white rounded-lg p-6 max-w-sm mx-auto text-center">
                ...
                <h3 class="text-xl font-semibold mb-2">Delete Enquiry?</h3>
                <p class="text-gray-600 mb-6">Are you sure you want to delete enquiry for <strong>{{ $enquiryToDeleteNumber }}</strong>?</p>
                <div class="flex justify-center gap-4">
                    <button wire:click="cancelDelete"
                        class="px-4 py-2 bg-gray-300 text-gray-800 rounded-md hover:bg-gray-400">Cancel</button>
                    <button wire:click="delete" wire:loading.attr="disabled"
                        class="px-4 py-2 bg-red-600 text-white rounded-md hover:bg-red-700 disabled:opacity-50 disabled:cursor-not-allowed">Yes, Delete</button>
                </div>
            </div>
        </div>
    @endif
